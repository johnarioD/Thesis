...
def train_baseline():
    ...
    for train_idx, test_idx in kf.split(X):
        ...
        model = BaselineModel(train_data.class_balance, should_transfer=False, model_type='simple_conv', im_size=imsize)
...
def train_SSL():
    ...
    # data
    print("Loading Data")
    X, Y = data.load_train_full(labeled_image_folder="data/preprocessed_hairy/BCC", unlabeled_image_folder="data/preprocessed_hairy/train", lbl_file="data/unprocessed/BCC_tags.csv", image_size=imsize)
    kf = KFold(n_splits=5)
    kf.get_n_splits(X)
